...
<div class="annuaire-entite">
        ...
        <div class="tags">
            @if ($annuaire->tags->isNotEmpty())
                ...
                @foreach ($annuaire->tags as $tag)
                    <a href="{{ route('annuaires.byTag', ['tag' => $tag->slug]) }}"
                        class="tag">{{ $tag->open }}</a>
                @endforeach
            @endif
        </div>
    ...
    <div class="entite-adresse">
        <p class="group">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" class="w-4 h-4 address">
                <path fill-rule="evenodd"
